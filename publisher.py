...
def publish_to_wordpress(article, wp_cfg):
    ...
    try:
        ...
        # 자가 수리(2026-09-02 실측, 카페24): 호스팅 방화벽이 본문 속 <script>(JSON-LD)를
        # XSS로 오인해 403(Apache 에러 페이지)을 돌려준다. ModSecurity를 꺼도 다른 계층이 막는다.
        # 403이고 본문에 script가 있으면 script 태그만 벗겨 1회 재시도한다(스키마는 Rank Math가 보완).
