...
- 분량·구조는 일반 글과 동일하게 지킨다(2026-09-02 실측: 커머스 모드 첫 글이 760자로 나와 품질 게이트 폐기됨):
  본문 1,800자 이상, H2 소제목 4개 이상(예: 유형 비교 / 상황별 선택 기준 / 비용·위약금 구조 / 신청·해지 절차 / 자주 하는 실수),
  '자주 하는 실수' 또는 '조건별 분기' 섹션을 반드시 포함하고, 구체 수치(가격 범위·기간·규격)를 4개 이상 넣는다.
- 실전 신호 표현을 본문에 그대로 최소 3회 이상 쓴다(품질 검사가 이 표현들을 기계로 찾는다):
  "흔한 실수", "주의할 점", "~라면 …이 낫다", "~인 경우에는", "반대로", "장단점", "비교하면".
  예: "원룸이라면 소형이 낫고, 반대로 4인 가구인 경우에는 대용량이 유리합니다. 장단점을 비교하면…"
- 다음 상투 표현은 절대 쓰지 않는다: "알아보겠습니다", "살펴보겠습니다", "이번 글에서는",
  "도움이 되셨기를", "마무리하겠습니다", "함께 알아봐요", "참고하시기 바랍니다".
- 제목 규칙: '알아야 할/알아보기/총정리/완벽 정리/하는 방법/확인하기/핵심 정보/한 번에/활용법' 금지.
  직전 글들과 같은 어미 반복 금지 — 구체 조건·수치가 든 문장형으로 매번 다르게
  (예: "원룸 정수기, 렌탈 3년 총비용 계산해보면 구매가 이깁니다").
"""
...
def _article_prompt(keyword, kind, category, links, related, insert_ads, competitive=False):
    ...
    ad_rule += (" [이미지 마커 작성법] 스타일은 object(정물 사진: 제품·도구 클로즈업 — 커머스 글 기본), "
                ...
                "⚠️ 장면묘사에는 이 글이 다루는 '핵심 제품·소재'의 명칭을 반드시 그대로 포함하고, "
